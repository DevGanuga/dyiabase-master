...
interface QuotesProps {
  ...
  onEditQuote?: (quote: AppQuote) => void
...
export function Quotes({ quotes, setQuotes, jobs, setJobs, userId, settings, onCreateQuote, onEditQuote, showSuccess, onOpenDyiaWithPrompt, isPro = false }: QuotesProps) {
  ...
  const convertAcceptedQuoteToJob = async (quote: AppQuote, jobDate?: string) => {
    try {
  ...
  return (
    <div className="page-content">
                ...
                <div className="flex flex-wrap gap-1">
                  {quote.status === 'draft' && (
                    <button
                      ...
                      className="text-xs px-2.5 py-1 rounded-lg bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition"
                      title="Mark as sent"
                    >
                      Mark Sent
                    </button>
                  ...
                  {(quote.status === 'sent' || quote.status === 'draft') && (
                    <button
                      ...
                      className="text-xs px-2.5 py-1 rounded-lg bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-900/50 transition"
                  ...
                  {quote.status === 'sent' && (
                    <button
                      ...
                      className="text-xs px-2.5 py-1 rounded-lg bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/50 transition"
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-0.5">
                    {onEditQuote && (
                      <button
                        ...
                        title="Edit quote"
                        aria-label={`Edit quote for ${quote.customer.name}`}
                      >
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {!quote.jobId && !isLinking && (
                      <button
                        ...
                        className="p-1.5 text-[var(--color-text-faint)] hover:text-blue-500 rounded-lg transition"
                        title="Link to job"
                        aria-label={`Link ${quote.customer.name}'s quote to a job`}
                      >
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101" />
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.172 13.828a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.102 1.101" />
                    ...
                    {quote.status === 'completed' && (
                      <button
                        ...
                        aria-label={`Request a review from ${quote.customer.name}`}
                      >
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="flex gap-0.5">
                    <button
                      ...
                      title="Download PDF"
                      aria-label={`Download PDF of ${quote.customer.name}'s quote`}
                    >
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button
                      ...
                      title="Delete"
                      aria-label={`Delete ${quote.customer.name}'s quote`}
                    >
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {convertingQuote && (
        <div className="modal-overlay" onClick={() => setConvertingQuote(null)}>
          <div className="modal-panel" onClick={(e) => e.stopPropagation()}>
            <h3 className="modal-title">Convert to Job</h3>
            <p className="modal-description">
              {convertingQuote.customer.name} accepted the quote for {formatCurrency(convertingQuote.total || Math.round((convertingQuote.estimateRange.low + convertingQuote.estimateRange.high) / 2))}. Choose the job date and confirm.
            </p>
            <div className="mb-4">
              <label className="block text-xs font-medium text-[var(--color-text-muted)] mb-1">Job Date</label>
              <input
                type="date"
                value={convertDate}
              ...
              <p className="text-xs text-[var(--color-text-faint)] mt-1">
                Set the date the work was or will be completed
              </p>
            </div>
            <div className="modal-actions">
              <button
                ...
                onClick={async () => {
                  ...
                  await convertAcceptedQuoteToJob(quote, convertDate)
                }}
