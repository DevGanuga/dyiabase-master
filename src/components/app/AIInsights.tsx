...
export function AIInsights({ type, className = '', compact = false, autoRefresh = false }: AIInsightsProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => fetchInsight(), 60 * 60 * 1000) // 1 hour
      ...
      <div className={`${className} animate-pulse`}>
        <div className={`bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 dark:from-orange-950/30 dark:via-amber-950/30 dark:to-yellow-950/30 border border-orange-200/50 dark:border-orange-800/30 rounded-xl ${compact ? 'p-4' : 'p-5 sm:p-6'}`}>
          <div className="flex items-start gap-3">
            <div className="w-8 h-8 bg-orange-200/50 rounded-lg flex-shrink-0" />
            <div className="flex-1 space-y-2">
              <div className="h-5 bg-orange-200/50 rounded w-3/4" />
              <div className="h-4 bg-orange-200/30 rounded w-full" />
              <div className="h-4 bg-orange-200/30 rounded w-2/3" />
            </div>
          </div>
        </div>
      </div>
  ...
  if (state.error) {
    // Silently fail for non-Pro users or show minimal error
    if (state.error.includes('Pro subscription') || state.error.includes('Unauthorized')) {
      return null
    }
    ...
    return (
      <div className={`${className}`}>
        <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
          <p className="text-sm text-slate-500">Unable to load AI insights</p>
        </div>
      </div>
    )
  }
  ...
  if (compact) {
    return (
      <div className={`${className}`}>
        <div className="bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 dark:from-orange-950/30 dark:via-amber-950/30 dark:to-yellow-950/30 border border-orange-200/50 dark:border-orange-800/30 rounded-xl p-4 sm:p-5 shadow-sm hover:shadow-md transition-shadow duration-300">
          <div className="flex items-start gap-3">
            {/* AI Icon */}
  ...
  return (
    <div className={`${className}`}>
      <div className="bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 dark:from-orange-950/30 dark:via-amber-950/30 dark:to-yellow-950/30 border border-orange-200/50 dark:border-orange-800/30 rounded-xl sm:rounded-2xl p-5 sm:p-6 shadow-sm">
        {/* Header */}
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Generated {state.generatedAt ? new Date(state.generatedAt).toLocaleString() : 'recently'}
                {state.cached && ' (cached)'}
              </p>
            </div>
          </div>
          <button
            onClick={() => fetchInsight(true)}
            className="p-2 text-slate-400 hover:text-orange-500 hover:bg-orange-100/50 rounded-lg transition-all"
            title="Refresh insight"
          >
