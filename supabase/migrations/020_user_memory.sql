...
CREATE INDEX idx_user_memory_user_id ON dyia_user_memory(user_id);
CREATE INDEX idx_user_memory_category ON dyia_user_memory(user_id, category);

-- Prevent exact duplicate memories per user
CREATE UNIQUE INDEX idx_user_memory_unique ON dyia_user_memory(user_id, lower(content));
